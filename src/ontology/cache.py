...
class OntologyCache:
    ...
    def lookup_exact(self, label: str, preferred_source: str | None = None) -> Optional[dict]:
        """Exact case-insensitive label match. When multiple terms share the
        label across ontologies (e.g. "skin" exists in both UBERON and MONDO),
        prefer the one matching `preferred_source`.
        """
        if preferred_source:
            row = self.conn.execute(
                "SELECT * FROM ontology_terms WHERE label = ? COLLATE NOCASE "
                "AND ontology_source = ? LIMIT 1",
    ...
    def lookup_synonym(self, term: str) -> Optional[dict]:
        ...
        FTS5 is the primary index. If FTS5 is available and runs cleanly,
        treat its empty result as authoritative — do NOT fall through to
        the 113K-row brute-force scan, which was costing 5+ minutes of
        wall time when resolving umbrella terms with 20+ children (Phase
        28.F profiling on R3-Q01 "covid lung").

        The brute-force path remains only as a guard against missing
        FTS5 indexes (e.g. older cache builds, in-memory test DBs).
        """
        ...
        try:
            ...
            pass  # FTS5 missing / malformed — fall through to brute force

        if fts5_ran:
            # FTS5 was authoritative; an empty result means "no match".
            return None

        # Brute-force fallback (rare path — FTS5 absent). Still slow but
        # at least correct on cache builds that predate the fts_ontology
        # virtual table.
