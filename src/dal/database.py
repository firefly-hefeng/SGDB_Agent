...
class DatabaseAbstractionLayer:
    ...
    # ID模式 → (表, 字段)
    # Phase 32: EGA accession patterns added — they hit unified_projects
    # directly because the id_mappings ETL doesn't backfill EGA entries.
    # E-MTAB-*/E-GEOD-*/etc are EBI series accessions; E-PROT-* / S-BSST*
    # are projects. They were previously falling through to the slow
    # id_mappings lookup and returning 404 when missing.
    ID_PATTERNS: dict[str, tuple[str, str]] = {
        ...
        "EGAS": ("unified_projects", "project_id"),
        "EGAD": ("unified_projects", "project_id"),
        "E-MTAB-": ("unified_projects", "project_id"),
        "E-GEOD-": ("unified_projects", "project_id"),
        "E-ENAD-": ("unified_projects", "project_id"),
        "E-HCAD-": ("unified_projects", "project_id"),
        "E-PROT-": ("unified_projects", "project_id"),
        "E-CURD-": ("unified_projects", "project_id"),
        "S-BSST": ("unified_projects", "project_id"),
        "S-HECA": ("unified_projects", "project_id"),
    }
    ...
    def get_entity_by_id(self, id_value: str) -> dict | None:
        ...
        # 5. Direct sample_id lookup — last resort so a clicked sample row whose
        #    project_id is NULL (EGA/HCA/PsychAD/HTAN, ~283K samples / 30% of the
        #    catalog, not present in id_mappings) resolves to its record (and via
        #    project_pk, its parent dataset) instead of 404-ing.
        result = self.execute(
            "SELECT * FROM unified_samples WHERE sample_id = ? LIMIT 1",
            [id_value.strip()],
        )
        if result.rows:
            return result.rows[0]
...
class SchemaInspector:
    ...
    def get_summary(self) -> dict:
        ...
        # 添加关键统计
        # Audit F16: in the current DB build the unified_* entities are VIEWS, not
        # tables, so they land in summary["views"] with no record_count and the
        # stats block reported 0 — contradicting /version (943,732 samples). Count
        # them live when the table-side count is absent/zero.
        tables = schema["tables"]

        def _count(name: str) -> int:
            rc = tables.get(name, {}).get("record_count", 0)
            if rc:
                return rc
            try:  # SchemaInspector holds the DAL as self._dal (it has no .execute)
                res = self._dal.execute(f"SELECT COUNT(*) AS c FROM {name}")
                return res.rows[0]["c"] if res.rows else 0
            except Exception:
                return 0

        summary["stats"] = {
            "total_projects": _count("unified_projects"),
            "total_series": _count("unified_series"),
            "total_samples": _count("unified_samples"),
            "total_celltypes": _count("unified_celltypes"),
            "total_entity_links": _count("entity_links"),
            "total_id_mappings": _count("id_mappings"),
        }
