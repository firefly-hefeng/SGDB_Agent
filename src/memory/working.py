...
class WorkingMemory:
    ...
    def add_turn(
        ...
        # Cache results
        cache_key = self._cache_key(query, self.session_id)
    ...
    def get_cached_result(self, query: ParsedQuery) -> list[dict] | None:
        ...
        key = self._cache_key(query, self.session_id)
    ...
    @staticmethod
    def _cache_key(query: ParsedQuery, session_id: str = "") -> str:
        """Deterministic cache key from query (defensive: also key on session_id)."""
        f = query.filters
        parts = [
            session_id,
            query.intent.name,
