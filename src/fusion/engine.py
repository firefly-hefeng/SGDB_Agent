...
class CrossDBFusionEngine:
    ...
    def fuse(
        ...
        # For very large result sets, skip entity_links (which issues one SQL per
        # 500-row batch). Rely on hash-based dedup only. Entity-link dedup is
        # most valuable for small result sets where cross-DB overlap matters.
        if len(results) > 2000:
        ...
        # Round-robin interleave by primary source so the top-K window spans
        # all available sources instead of being dominated by whichever source
        # the SQL query happened to scan first. Only kicks in when the result
        # involves multiple sources and is large enough that pagination matters.
        if len(sources) > 1 and len(fused) > 50:
            fused = self._interleave_by_source(fused)

        return fused

    @staticmethod
    def _interleave_by_source(records: list[FusedRecord]) -> list[FusedRecord]:
        from collections import OrderedDict
        buckets: OrderedDict[str, list[FusedRecord]] = OrderedDict()
        for r in records:
            key = (r.sources[0] if r.sources else "unknown")
        ...
        for i in range(0, len(pks), BATCH):
            ...
            try:
                ...
                for link in links.rows:
                    ...
                    if sp in pks_set and tp in pks_set:
                        uf.union(sp, tp)
            except Exception as e:
                logger.warning("entity_links query failed (dedup may be incomplete): %s", e)
    ...
    def _merge_by_hash(self, groups: list[list[dict]]) -> list[list[dict]]:
        """利用biological_identity_hash进一步合并 — 仅跨数据源合并。

        Within the same source_database, different sample_ids with the same
        biological_identity_hash are still kept separate (they're distinct
        accessions in that repo). We only merge hash-collisions when the
        records come from different sources — that's genuine cross-DB
        redundancy.
        """
        hash_to_groups: dict[str, list[int]] = {}
        merged_groups: list[list[dict]] = []

        def _sources_of(group: list[dict]) -> set[str]:
            return {r.get("source_database") or r.get("sample_source") or "" for r in group}
        ...
        for group in groups:
            ...
            if h and h in hash_to_groups:
                # Try to find an existing group that doesn't share any source
                # with the incoming group — that's the cross-DB merge case.
                incoming_sources = _sources_of(group)
                merged = False
                for idx in hash_to_groups[h]:
                    if not (_sources_of(merged_groups[idx]) & incoming_sources):
                        merged_groups[idx].extend(group)
                        merged = True
                        break
                if not merged:
                    # Same source as an existing same-hash group → keep separate
                    idx = len(merged_groups)
                    merged_groups.append(group)
                    hash_to_groups[h].append(idx)
            else:
                idx = len(merged_groups)
                merged_groups.append(group)
                if h:
                    hash_to_groups[h] = [idx]
