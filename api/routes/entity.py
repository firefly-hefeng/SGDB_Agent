...
async def get_entity(id_value: str):
    ...
    if entity is None:
        # Phase 32: don't echo arbitrary user input in error messages
        # (defence-in-depth against reflected XSS in tooling that
        # parses our errors as HTML). The id is available to the
        # caller in the request path; no need to repeat it.
