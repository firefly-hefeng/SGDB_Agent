...
def _export_csv(records, fields: list[str] | None) -> StreamingResponse:
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=singligent_export.csv"},
    )
...
def _export_json(records, fields: list[str] | None, summary: str) -> StreamingResponse:
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=singligent_export.json"},
    )
...
def _export_bibtex(records) -> StreamingResponse:
    ...
    for i, r in enumerate(records):
        pmid_raw = r.data.get("pmid") or r.data.get("project_pmid")
        ...
        # Audit F17: a PMID is all-digits. Some records carry a DOI mis-stored in
        # the pmid column — emitting it as `pmid = {10.x/...}` is an invalid entry.
        # Validate, and recover a misfiled DOI into the doi field.
        pmid = None
        if pmid_raw:
            s = str(pmid_raw).strip()
            if s.isdigit():
                pmid = s
            elif not doi and (s.startswith("10.") or "/" in s):
                doi = s
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=singligent_export.bib"},
    )
