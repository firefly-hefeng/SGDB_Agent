...
# Audit F1-F3 (Phase 36): tissue/disease/cell_type facet COUNTS and STRUCTURED
# FILTERS must both hit the cleaned *_standard columns — the same ones the
# empty-state precomputed facets are built from (explore.STANDARD_FACETS). Before
# this, facets were counted over *_standard but filtered over the raw column, so a
# facet click returned ~2% of the advertised count and disagreed with /explore.
FACET_FIELDS = {
...
# Structured-filter fields that resolve to a cleaned *_standard column.
_STD_COLUMN = {
    "tissue": "tissue_standard",
    "disease": "disease_standard",
    "cell_type": "cell_type_standard",
}


def _std_in_clause(field: str, values: list[str]) -> tuple[str, list]:
    """Index-friendly canonical match against a *_standard column (reusing the
    same facet vocabulary explore uses), with a case-folded fallback when the
    vocabulary is cold or a value is unknown — so correctness never depends on
    the resolver being warm."""
    col = _STD_COLUMN[field]
    try:
        from api.routes.explore import _canon_map
        cmap = _canon_map(field)
    except Exception:  # pragma: no cover — defensive
        cmap = {}
    canon = [cmap.get(v.strip().lower()) for v in values]
    if cmap and all(c is not None for c in canon):
        ph = ",".join("?" for _ in canon)
        return f"s.{col} IN ({ph})", canon
    ph = ",".join("?" for _ in values)
    return f"LOWER(TRIM(s.{col})) IN ({ph})", [v.strip().lower() for v in values]
...
def parsed_query_to_conditions(parsed) -> list[ParsedCondition]:
    ...
    mapping = [
        ...
        ("development_stage", getattr(f, "development_stages", None)),
...
def conditions_to_where(
    ...
    for c in conditions:
        if c.field == "tissue" and c.values:
            clause, p = _std_in_clause("tissue", c.values)
            clauses.append(clause)
            params.extend(p)
        elif c.field == "disease" and c.values:
            clause, p = _std_in_clause("disease", c.values)
            clauses.append(clause)
            params.extend(p)
            ...
            clause, p = _std_in_clause("cell_type", c.values)
            clauses.append(clause)
            params.extend(p)
            ...
            params.extend(c.values)
        elif c.field == "development_stage" and c.values:
            placeholders = ",".join("?" for _ in c.values)
            clauses.append(f"s.dev_stage_category IN ({placeholders})")
            params.extend(c.values)
            ...
            safe_q = safe_fts5_query(c.values[0])
            if safe_q:
                clauses.append(
...
async def advanced_search(req: AdvancedSearchRequest):
    ...
    if req.nl_query and req.nl_query.strip():
        ...
        # Route NL queries through full coordinator pipeline (parse → enrich → ontology → SQL → execute → fuse → synthesize)
        # This works with any parser (rule-based or LLM) — the enricher adds LLM intelligence
        #
        # R2-2 (refine-within-results): when the caller already carries explicit
        # conditions (an existing filtered range), we DON'T take the fresh-search
        # agent path — instead we parse the NL into conditions and merge them
        # with the incoming ones, then run the structured path below, which ANDs
        # every condition. So "breast cancer" + then "T cells" = breast-cancer
        # AND T-cell, rather than the NL query silently replacing the range.
        _use_agent_pipeline = coordinator is not None and not req.conditions

        if _use_agent_pipeline:
            # Full agent pipeline: parse → enrich → ontology → SQL → execute → fuse → synthesize
            try:
                ...
                # Phase 27: surface the full step-by-step reasoning trace so the
                # UI can render the complete execution flow (the coordinator
                # already builds it; previously it was discarded here).
                provenance.reasoning_trace = agent_response.provenance.reasoning_trace
                ...
                # Bug 3 fix: agent pipeline must surface parsed filters back as
                # ParsedCondition chips so the frontend can display + edit them.
                # Previously this branch returned conditions=conditions (the
                # caller's input) verbatim, hiding what the agent actually
                # parsed. We rebuild conditions from agent_response provenance.
                _agent_parsed = getattr(agent_response, "_parsed_query", None)
                if _agent_parsed is not None:
                    new_conditions = parsed_query_to_conditions(_agent_parsed)
                    conditions = merge_conditions(conditions, new_conditions)
                else:
                    # Fallback: reconstruct from provenance.parsed_filters dict
                    pf = agent_response.provenance.parsed_filters or {}
                    if pf:
                        from types import SimpleNamespace
                        # Build a ParsedQuery-shaped object good enough for
                        # parsed_query_to_conditions.
                        _filters = SimpleNamespace(
                            tissues=pf.get("tissues") or [],
                            diseases=pf.get("diseases") or [],
                            organisms=pf.get("organisms") or [],
                            assays=pf.get("assays") or [],
                            cell_types=pf.get("cell_types") or [],
                            source_databases=pf.get("source_databases") or [],
                            project_ids=pf.get("project_ids") or [],
                            sample_ids=pf.get("sample_ids") or [],
                            pmids=pf.get("pmids") or [],
                            sex=pf.get("sex"),
                            min_cells=pf.get("min_cells"),
                            has_h5ad=pf.get("has_h5ad"),
                            free_text=pf.get("free_text"),
                        )
                        _pq_proxy = SimpleNamespace(
                            filters=_filters,
                            confidence=0.85,
                        )
                        try:
                            new_conditions = parsed_query_to_conditions(_pq_proxy)
                            conditions = merge_conditions(conditions, new_conditions)
                        except Exception as _ex:
                            logger.debug("conditions reconstruction skipped: %s", _ex)

                # Phase 27 (A6): the agent returns its top-N (≤100) ranked rows,
                # but the UI paginates at `limit` (25). Returning all 100 made
                # the header read "1–25 of N" while 100 rows rendered. Slice to
                # the requested page for a coherent first page; facets above are
                # still computed from the full ranked set. Page 2+ flow through
                # the structured (offset-based) path.
                page_records = records[req.offset:req.offset + req.limit] if req.limit else records

                # Phase 33: surface aggregation output for STATISTICS queries
                # ("count by …"). The agent returns grouped rows + a chart; the
                # search-record extraction above maps them to empty sample rows,
                # so without this the UI showed blank rows. Carry the chart(s)
                # and the raw grouped rows so the frontend can render a breakdown.
                charts_out = [
                    ChartOut(type=c.type, title=c.title, data=c.data)
                    for c in (agent_response.charts or [])
                ]
                aggregation_rows: list[dict] = []
                if agent_response.provenance.parsed_intent == "STATISTICS" or charts_out:
                    aggregation_rows = [
                        dict(r.data) for r in agent_response.results
                        if getattr(r, "data", None) and "count" in r.data
                    ]
    ...
    if where_clauses:
        ...
        from api.routes.explore import _load_precomputed_facets, _unfiltered_facets
