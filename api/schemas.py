...
class ProvenanceOut(BaseModel):
    original_query: str = ""
    parsed_intent: str = ""
    # Phase 38: the structured filters the agent extracted (tissues, diseases,
    # cell_types, assays, …). The coordinator always builds these; exposing them
    # lets the frontend show "what I understood" — essential when a query returns
    # 0 rows (e.g. PBMC + 10x: the user sees tissue=PBMC, assay=10x were parsed,
    # so the "drop assay" broaden-suggestion is intelligible rather than mysterious).
    parsed_filters: dict = {}
    ...
    # Phase 27: the full step-by-step reasoning trace (parse → ontology →
    # sql_gen → execute → fuse → synthesize, with per-stage timings, rationale,
    # SQL, and self-corrections). The coordinator already builds this; surfacing
    # it lets the frontend render the complete execution flow in a collapsible
    # panel instead of a fake client-side progress timer.
...
class ResultRecord(BaseModel):
    ...
    # Phase 22-D: per-facet hit/miss explanation
    facet_match: dict[str, str] = {}
...
class AdvancedSearchResponse(BaseModel):
    ...
    provenance: ProvenanceOut = ProvenanceOut()
    suggestions: list[SuggestionOut] = []
    # Phase 33: aggregation output (bar/pie group→count) for STATISTICS queries
    # like "count samples by disease category" — previously dropped, so the UI
    # showed empty sample rows instead of the breakdown.
    charts: list[ChartOut] = []
    # Phase 33: grouped rows for aggregation queries (each = {label, count, …}).
